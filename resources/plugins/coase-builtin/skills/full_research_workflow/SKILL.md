...
1. **研究目的**：上下文里有 `research_purpose: causal|associative` 字段（由"研究设置"注入）。所有下游 skill 都按这个字段分支，不要擅自默认为 causal。
2. **评审模型组**：**不要自己去翻 `settings.json` / `providers.json` 或任何本地配置文件来验证是否配置**——这些文件的真实存储路径和字段名不在你能稳定猜到的位置（配置其实存在 `{userData}/config/providers.json` 的 `criticPanelIds` 字段，你翻 settings.json 只会扑空）。正确做法：**直接按下面的推荐顺序往下走**，等调 `idea-critic` / `paper-reviewer` 时，底层 `mcp__coase-critic-panel__invoke` tool 会自己返回 `isError=true` 告诉你"评审模型组尚未配置"，那时再停下来让用户去设置页勾选。**没有预检查这一步。**
