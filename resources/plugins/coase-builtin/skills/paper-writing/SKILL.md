...
- **/idea-discovery**: 不直接参与（planner 不写正文）。
...
- **No `\titlepage` environment** — it forces abstract to a separate page
- **No `\tableofcontents`** — not standard in economics journal submissions
- **`\begin{spacing}{1.0}` wrapping the title block** — the global 1.5x stretch would otherwise inflate the title's line spacing
- **`\thispagestyle{empty}`** — suppresses page number on cover page
- **Title font: `\large\bfseries`** not `\LARGE`** — `\LARGE` combined with 1.5x stretch consumes too much vertical space
...
- **literature-review**: Conduct the literature review that feeds into the paper
- **/method**: Generate the methodology section with proper econometric notation
- **table**: Format regression tables and summary statistics for the paper
- **beamer-ppt**: Convert the paper into a conference presentation
- **/interpret**: Get plain-language interpretation of results for the paper
