...
- 工作目录：`review/`
- 必须调用用户在设置页选定的"评审模型组"（provider 多选）。主模型是被评的角色，评审模型提供独立第二视角。panelSize=1 走单 referee 模式，panelSize≥2 走多 referee 共识/分歧模式
...
- 不得替用户改稿，只输出意见和建议
- 不得擅自补跑回归或实验
- panelSize≥2 时评审结论必须基于多模型聚合，不得让单一 critic 决定；panelSize=1 时直接给出该 critic 的意见清单（单 referee 模式），并在输出里明确标注"本次仅 1 个独立评审模型，建议用户补配更多 provider 以进入对抗共识模式"
- 若用户 0 个都没配（panelSize=0），停下来提示用户前往设置 → 评审模型组勾选至少 1 个独立 provider，让用户补配
