---
name: data-cleaning
description: Clean, validate, and document raw datasets into reproducible analysis samples
---
...
- Typical role in the Coase workflow: Use this immediately after obtaining raw data to rename variables, handle missingness, remove duplicates, merge sources, and define the estimation sample.
...
- 鉂?Not checking for duplicates before merging
- 鉂?Forgetting to handle missing value codes (-99, -88, etc.)
- 鉂?Not labeling variables and values
- 鉂?Overwriting raw data files
- 鉂?Not documenting data transformations
...
print(f"Saved {len(df):,} rows 脳 {df.shape[1]} columns")
```
...
cat(sprintf("Saved %d rows 脳 %d columns\n", nrow(df), ncol(df)))
```

## Working With Other Coase Skills

- This often follows `data-fetcher`.
- After cleaning, continue into `stats`, `ols-regression`, `panel-data`, or another estimation skill.
- Do not rely on the old `/analyze` command; produce scripts, logs, and a clear cleaning record directly.
