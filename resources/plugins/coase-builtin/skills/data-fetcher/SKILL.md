---
name: data-fetcher
description: Fetch economics and finance data from public datasets, APIs, and web sources
---
...
- Typical role in the Coase workflow: Use this once the research question and variable definitions are clear, and record source, frequency, field definitions, update time, and fetch scripts.
...
def fetch_fred_series(
    ...
    for series_id in series_ids:
        try:
            ...
            print(f"鉁?Downloaded {series_id}")
        except Exception as e:
            print(f"鉁?Failed to download {series_id}: {e}")
...
def fetch_world_bank_data(
    ...
    for indicator_code, indicator_name in indicators.items():
        try:
            ...
            print(f"鉁?Downloaded {indicator_name}")
            
        except Exception as e:
            print(f"鉁?Failed to download {indicator_name}: {e}")
...
- 鉂?Hardcoding API keys in scripts
- 鉂?Not handling API rate limits
- 鉂?Ignoring data vintages/revisions
- 鉂?Mixing data frequencies without proper handling

## Working With Other Coase Skills

- Hand fetched data to `data-cleaning` immediately after collection.
- If the task is literature-driven data sourcing, pair with `literature-review` to identify standard datasets first.
- Do not rely on the old `/analyze` command; use the web, file, and shell tools that Coase already exposes.
