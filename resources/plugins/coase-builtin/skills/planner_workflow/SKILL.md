...
- 数据规模决定读取与扫描策略，详见下方"步骤 0"，不得无视规模直接整文件 Read
...
**禁止**：
- 把原始 CSV/DTA 整文件 Read 到上下文
- 把大数据集的 `summary()` 全量或 > 100 × 100 的相关系数矩阵粘进对话
- 对 > 100 列的数据集一次性输出所有列的描述性统计——先按主题/角色分组
...
> **联网搜索文献开关**：如果 system prompt 中 `web_search_enabled: false`（或出现"联网搜索文献：关闭"硬约束句），**整个 Phase 2 不得使用 WebSearch / WebFetch 检索文献**——这两个工具在文献场景下已被 PreToolUse hook 直接 deny。改为按下方"本地优先模式"执行。
...
本地优先模式（联网搜索文献关闭时）：
- 仅使用上下文中的"可用参考文献（references/）"列表读取本地 PDF；先前在"读取参考文献"小节里已抽取过的核心发现 / 识别策略 / 数据变量直接复用
- 综合分析：基于本地文献得到的发现、可推断的 Gap（明确写"基于本地文献样本，研究空白可能为 X，待联网补充验证"）、与数据契合度
- 如果 references/ 完全为空：在 stage_2_literature.md 顶部如实写"本地无可用文献，按用户研究偏好未联网补齐"，文献表留占位，仍输出基于研究问题的方向建议，**不要尝试绕过限制**
...
## Phase 9: 用户确认关卡（HITL，强制执行）

**目的**：Planner 的产出直接决定 Executor 会跑什么回归、Reviewer 会审什么方案。摘要落盘不等于用户认可——**必须在这里暂停，让用户亲自确认或提出修改**，再由上层编排器进入下一 skill。

### 硬性规则

1. **输出最终摘要后立即停止本 skill 的执行**。不得在同一轮对话里调用 paper-reviewer、executor_workflow 或任何下游 skill，也不得自行进入 Phase 1 重做。
2. 摘要末尾**必须**附一段"等待用户确认"提示，明确列出三种选项：
   - **A. 确认方案**：用户说"确认 / 通过 / 可以进入评审"之类明确授权的话 → 控制权交还给编排器，进入 paper-reviewer。
   - **B. 局部修改**：用户指出具体想改的 Phase（例如"Phase 7 改用 IV"、"Phase 3 假设换成 X"），Planner 只回炉被指出的 Phase，**覆盖或追加对应 `stage_*.md`**，其他已通过的 Phase 不重跑；修改完成后**再次回到 Phase 9**等待确认。
   - **C. 整体重做**：用户要求推翻重来，则从 Phase 1 重新走一遍，所有 `stage_*.md` 按需覆盖。
3. **判定"确认"的标准**：用户明确的肯定表述（"确认 / 同意 / 通过 / 进入下一步 / 可以开始跑"等）。用户沉默、含糊回复、只是点头式的"嗯"都**不算**确认，需追问"是否确认进入 paper-reviewer 评审？"。
4. **严禁行为**：
   - ❌ 自己替用户决定"方案已经够好了，直接进 Reviewer"。
   - ❌ 把用户"我再想想"、"先这样吧"当作 A 选项的确认信号。
   - ❌ 在未得到确认前把 baseline memo 的 `status` 从 `draft / partial` 改为 `locked`。
