...
description: Run synthetic-control, placebo, donor-weight, and synthetic-DID workflows
---
...
- Typical role in the Coase workflow: Use this when treated units are few, intervention timing is clear, and a synthetic comparison is appropriate.
...
**Estimand**: 蟿_t = Y鈧佲倻 鈭?哦鈧佲倻^(SC) for post-treatment periods t > T鈧€
...
4. **Estimate effects**: Gap = treated outcome 鈭?synthetic control outcome
5. **Inference**: Run placebo tests (in-space and in-time)
6. **Report**: Gap plot, placebo plot, MSPE ratios

## Code Templates

### R 鈥?Classic SCM (Synth package)

```r
# R 鈥?Abadie-Diamond-Hainmueller SCM
...
gaps.plot(synth_out, dataprep_out,
          Ylab = "Gap (Treated 鈭?Synthetic)", Xlab = "Year",
          Main = "Treatment Effect Over Time")
abline(v = 2000, lty = 2, col = "red")
abline(h = 0, lty = 3)
```

### R 鈥?Augmented SCM (augsynth)

```r
# R 鈥?Augmented SCM (Ridge-augmented, Ben-Michael et al. 2021)
...
```python
# Python 鈥?SparseSC (penalized synthetic control)
...
# Reshape data to (N_units 脳 T_periods) matrix
...
### Stata 鈥?synth and synth_runner
...
```stata
* Stata 鈥?in-space placebo with synth_runner
...
# p-value: fraction of placebos with ratio 鈮?treated
...
# Gap should be 鈮?0 if model is well-specified
...
```r
# R 鈥?predictor balance check after synth()
...
```python
# Python 鈥?predictor balance after SparseSC or manual SCM
...
synthetic_pred = (weights @ X_donors).flatten()  # weights 脳 donor covariates
...
# Flag predictors where |Pct_Gap| > 5% 鈥?consider adjusting donor pool
...
```stata
* Stata 鈥?predictor balance displayed automatically after synth
synth outcome gdp_pc trade_share inflation ///
    outcome(1995) outcome(1990) outcome(1985), ///
    trunit(1) trperiod(2000)
...
```r
# R 鈥?compute and assess pre-treatment RMSPE
...
if (pre_rmspe / outcome_mean > 0.05) {
  warning("Pre-treatment RMSPE exceeds 5% of outcome mean 鈥?fit may be poor.")
}
```

```python
# Python 鈥?pre-treatment fit visualization
...
axes[1].plot(years, gap, 'b-', linewidth=2, label='Gap (蟿虃)')
...
```stata
* Stata 鈥?pre-treatment fit assessment
...
- **Poor pre-treatment fit**: If pre-MSPE is large, the synthetic control is unreliable 鈥?reconsider predictor set or donor pool
- **Overfitting to noise**: Using too many outcome lags can overfit; use 3鈥? evenly spaced lags
...
- This usually follows `data-cleaning` so the donor pool and panel structure are explicit.
- Outputs typically flow into `figure`, `table`, and `paper-writing`.
- Do not rely on old `/robustness`, `/method`, or `/plot` commands; provide diagnostics and visuals directly.
