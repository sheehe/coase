---
name: figure
description: Produce publication-grade figures, event-study plots, mechanism diagrams, and presentation visuals
---
...
- Typical role in the Coase workflow: Use this when results need to be explained visually in papers, appendices, or presentations.
...
| Dimensions | Width: 3.4in (single column) or 7in (full page); height 鈮?9in |
| Font | Matching journal body font; minimum 8pt for labels |
| Colors | Must be readable in grayscale; avoid red-green pairs |
| Line width | 鈮?0.5pt for data lines; 鈮?0.75pt for axes |
...
```python
# Python 鈥?journal-quality defaults
...
```r
# R 鈥?journal-quality ggplot2 theme
...
```stata
* Stata 鈥?journal-quality graph scheme
set scheme s2color
...
```python
# Python 鈥?coefficient plot comparing specifications
...
```r
# R 鈥?coefficient plot with modelsummary/modelplot
...
```python
# Python 鈥?binned scatter with linear fit
...
```r
# R 鈥?binscatter with binsreg (Cattaneo et al.)
...
```python
# Python 鈥?RDD plot with local polynomial fit
...
```python
# Python 鈥?overlapping density plot
...
```python
# Python 鈥?time series with shaded recession bars
...
```r
# R 鈥?time series with recession shading
...
```python
# Python 鈥?treated vs synthetic control + gap
...
def plot_synth(years, treated, synthetic, treatment_year, title=""):
    ...
    ax.set_ylabel("Treated 鈭?Synthetic")
...
```r
# R 鈥?multi-panel with patchwork
...
```stata
* Stata 鈥?multi-panel with graph combine
...
- **Using default matplotlib/ggplot themes**: They look unprofessional 鈥?always customize
- **Raster exports for line plots**: Use PDF/EPS, not PNG, for any plot with lines or text
- **Too many colors**: Limit to 3鈥? distinguishable colors; use linestyle for additional series
...
- **3D plots**: Almost never appropriate in economics 鈥?use 2D alternatives
...
```latex
% In preamble 鈥?point to figures/ once:
\graphicspath{{../figures/}}

% In the paper body 鈥?no path needed in each call:
\begin{figure}[htbp]
  \centering
...
- Inputs usually come from `stats`, `did-analysis`, `iv-estimation`, `panel-data`, and related estimation skills.
- Outputs typically flow into `table`, `paper-writing`, and `beamer-ppt`.
- Do not rely on the old `/plot` command; treat figures as direct research artifacts.
