...
description: Planner 8 阶段推荐规划流程：数据对齐→文献探索→假设生成→质量关卡(迭代回溯)→变量确认→数据支撑→基准设计锁定→描述性统计
metadata:
  agents: [planner]
---
...
以下是推荐的规划流程，供参考。你可以根据实际情况调整，必要时回溯前序步骤或跳过不适用的内容。
...
**下游 Reviewer 和 Executor 按固定文件名读取你的输出**。每个 Phase 结束前**必须**调用 `write_file` 写入对应文件，否则等同于没做。
...
**硬性规则**：
- 每个 Phase 的"输出"段说的文件名就是 `write_file` 的 `rel_filepath`，一字不差
...
在开始任何 Phase 之前，先查看上下文中的 **入口模式（entry_mode）**，按对应说明调整执行路径：

| 模式 | 含义 | 执行调整 |
|------|------|---------|
| **A** | 从零开始 | 按默认顺序执行 Phase 1 → 8 |
| **B** | 已有参考文献 | Phase 1 完成后，读取 references/ 下的 PDF，再进入 Phase 2 |
| **C** | 已有初步研究方案 | Phase 1 之前先读取 plans/ 下的文档，再按正常顺序 Phase 1 → 8 |
| **D** | 研究方案已完整 | Phase 1 之前先读取 plans/ 下的文档，Phase 5 变量确认完成后可跳过 Phase 2、3，直接进入 Phase 6 |

### Mode B：读取参考文献

执行时机：**Phase 1 完成之后、进入 Phase 2 之前**。

步骤：
1. 查看上下文"可用参考文献（references/）"列表，确认有哪些 PDF 文件
2. 对每个 PDF 调用 `parse_document(filepath, max_pages=30)` 读取正文
...
2. 对每个文件调用 `parse_document(filepath)` 或 `read_file(filepath)` 读取内容
3. 理解用户已有的：研究问题、核心假设、变量设计、预期识别策略
4. 以此作为 Phase 1 数据对齐的起点（重点确认方案与数据的匹配程度，而非从零生成方向）

Mode D 额外说明：若读取的方案已明确给出变量映射且数据支持充分，Phase 5 完成后可直接跳至 Phase 6，无需执行 Phase 2、3。
...
**步骤 1（必须）**：对每个数据文件调用 `get_dataset_columns(file_path)`
- 获取全部列名和变量标签（Stata DTA 的 variable label 含业务含义，如 b001→营业收入）
- 理解数据集的完整变量列表后，才能进行后续分析

**步骤 2（必须）**：识别结构性维度并全量扫描
- 识别疑似实体ID列（列名含 id/code/stkcd/firmid/企业 等，或字符型/高基数整数），调用 `scan_column(file_path, id_col, mode="full")`
  → 获得**精确**实体数量（不能靠估算，这决定研究样本规模）
- 识别疑似时间列（列名含 year/date/time/period/年/月 等，或取值在 1990-2040 的整数），调用 `scan_column(file_path, time_col, mode="full")`
  → 获得完整时间跨度和时间粒度（决定面板长度）

**步骤 3（按需）**：对研究核心变量调用 `scan_column(file_path, col, mode="sample")`
- 了解变量分布、缺失率，判断是否满足识别假设
- 如数据字典已存在，可先调用 `read_data_dictionary` 补充语义信息
...
**落盘（必须）**：调用 `write_file(workspace_dir, "planner/stage_1_alignment.md", content)` 把 A-E 全部写入。
...
推荐方法：
- 根据研究方向提炼2-3个搜索关键词组合
- 使用 `search_semantic_scholar` 搜索相关文献（每组关键词5-10篇）
- 对最相关的2-3篇论文使用 `expand_citations` 扩展引文网络（深度1层）
- 综合分析：现有研究主要发现、研究空白（Gap）、可用识别策略、与数据的契合度

**输出**：关键文献列表（作者、年份、标题、核心发现、识别策略）、研究空白分析、推荐2-3个研究方向（按可行性排序）、每个方向适合的识别策略

**落盘（必须）**：调用 `write_file(workspace_dir, "planner/stage_2_literature.md", content)` 把文献表 + Gap + 方向全部写入。文献的完整 abstract **只落盘不粘对话**。
...
**落盘（必须）**：调用 `write_file(workspace_dir, "planner/stage_3_hypothesis.md", content)`。注意是 `planner/stage_3_hypothesis.md`，不要漏了 `planner/` 前缀。
...
无论是否触发回溯，都使用 `write_file(workspace_dir, "planner/stage_4_quality_gate.md", content)` 写入（完整路径，含 `planner/` 前缀）。**回溯时追加 Iteration 记录，不覆盖历史**（先 `read_file` 读出现有内容，拼接后再写）：
...
**落盘（必须）**：调用 `write_file(workspace_dir, "planner/stage_5_variable_mapping.md", content)`。
...
**落盘（必须）**：调用 `write_file(workspace_dir, "planner/stage_6_data_support.md", content)`。
...
**落盘（必须、不可跳过）**：调用 `write_file(workspace_dir, "planner/stage_7_baseline_design.md", content)` 写入完整 baseline memo。

**这一步是 Executor 的唯一输入文件**，Executor 会用 `read_file("planner/stage_7_baseline_design.md")` 读取；如果你不写这个文件，整个后续流水线直接停摆。即使当前数据只 "partially supported"、即使你还拿不定主意，**也必须写一份 draft 版本**并在文件头部标注：
...
推荐方法：
- 对每个核心变量调用 `scan_column(file_path, col, mode="sample")` 获取分布统计量（均值/分位数/缺失率）
...
**落盘（必须）**：调用 `write_file(workspace_dir, "planner/stage_8_descriptive_snapshot.md", content)`。

---

## 最终自检（所有 Phase 结束后执行）

在给出最终摘要**之前**，调用 `list_workspace_files(workspace_dir, subdir="planner")` 并核对以下 8 个文件是否都已生成：

- [ ] `planner/stage_1_alignment.md`
- [ ] `planner/stage_2_literature.md`（Mode B/C/D 跳过的也要写占位）
...
任何一个缺失 → **立刻补写**再输出最终摘要。如果确实某 Phase 被合理跳过（如 Mode D 跳过 Phase 2/3），也要写占位文件，文件内容说明跳过原因 + `status: skipped`。

完成核对后，提供一份完整的研究计划摘要（不超过2000字），涵盖：研究问题、假设、识别策略、基准方程、变量列表、稳健性检验计划。这份摘要将作为 Planner 的最终输出，传递给 Reviewer 审阅。
