...
description: Executor 5 阶段推荐执行流程：数据准备→基线回归→稳健性检验→表格图表输出→产出评估
metadata:
  agents: [executor]
---
...
以下是推荐的执行流程，供参考。你可以根据实际情况调整顺序、回溯或跳过不适用的步骤。

---

## 文件落盘契约（每个 Phase 必须落盘）

下游 Reviewer（Mode B）和 Writer 按固定文件名读取你的输出。每个 Phase 结束前**必须**调用 `write_file` 写入对应的 stage_*.md，否则等同于没做。

> ### ⚠️ 路径警示（极易写错，违反下游契约）
>
> **`stage_*.md` 必须落在 `executor/` 根目录**，绝对不能写到 `executor/scripts/` 或其他子目录。
> `executor/scripts/` 只允许存放 `.R` 脚本（由 `write_r_script` 工具自动管理）。
>
> ✅ 正确：`write_file(workspace_dir, "executor/stage_2_run_baseline.md", content)`
> ❌ 错误：`write_file(workspace_dir, "executor/scripts/stage_2_run_baseline.md", content)` ← 路径多了一级 `scripts/`
> ❌ 错误：`write_file(workspace_dir, "scripts/stage_2_run_baseline.md", content)` ← 漏了 `executor/` 前缀
>
> 调用 `write_file` 前自查一次 `rel_filepath`：是否正好是 `executor/stage_X_*.md` 的形式？多一级或少一级都会让 Reviewer 找不到文件。
...
- 使用 `read_file` 读取基准设计文件（planner/stage_7_baseline_design.md），理解所需变量和回归规格
- 使用 `write_r_script` 创建 `01_data_preparation.R`：加载必要包（data.table, haven, fixest 等）、读取原始数据、数据清洗、变量构造、保存清洗后的分析数据集
- 使用 `execute_r_in_docker` 执行脚本
- 使用 `list_workspace_files` 检查输出文件是否生成，确认关键变量存在且类型正确
...
**落盘（必须）**：调用 `write_file(workspace_dir, "executor/stage_1_data_preparation.md", content)` 记录：
...
### Phase 2: Run Baseline（PDF Phase 4）

**目标**：执行主回归，产出可信赖的基准结果。

推荐方法：
- 重新阅读基准设计（planner/stage_7_baseline_design.md），严格按设计执行
- 使用 `write_r_script` 创建 `02_baseline_regression.R`，生成清晰、可运行、注释简洁的 R 代码
- 使用 `execute_r_in_docker` 执行；如失败，优先做最小修改修复技术错误（用 `file_edit`），不得随意改变研究设计
...
**落盘（必须）**：
- `write_file(workspace_dir, "executor/stage_2_run_baseline.md", content)` 写入 1-4 项（自然语言总结 + 表格路径引用）
- `write_file(workspace_dir, "executor/specification_log.md", content)` 追加第 5 项（追加时先 `read_file` 读现有内容再拼接，不要覆盖）

---

### Phase 3: Explanation Check & Robustness（PDF Phase 5）
...
- 使用 `write_r_script` + `execute_r_in_docker` 执行
- 失败时做最小技术修复，不得改变检验逻辑

**输出格式**：Priority Check Map、Mechanism-Supporting Evidence（最多3个）、Heterogeneity（最多3个）、Robustness Checks（最多5个）、Overall Assessment

**落盘（必须）**：
- `write_file(workspace_dir, "executor/stage_3_explanation_robustness.md", content)` 写入全部 5 个部分
- 追加到 `executor/specification_log.md`（同 Phase 2，读后拼接再写）

---

### Phase 4: Table & Figure Output（PDF Phase 6 Steps 1-3）
...
- 使用 `write_r_script` + `execute_r_in_docker` 生成表格
...
**落盘（必须）**：调用 `write_file(workspace_dir, "executor/stage_4_table_figure_output.md", content)`。文件内需列出：
...
### Phase 5: Output Assessment（PDF Phase 6 Step 5）
...
**落盘（必须）**：调用 `write_file(workspace_dir, "executor/stage_5_assessment.md", content)`。

---

## 最终自检（所有 Phase 结束后执行）

在给出最终总结**之前**，调用 `list_workspace_files(workspace_dir, subdir="executor")` 核对以下文件：
...
完成核对后，提供 Executor 阶段的完整总结（不超过2000字），涵盖：基准回归结果、关键发现、稳健性检验结论、生成的表格和图表清单、Specification Log 摘要、对 Writer 的建议。
