...
## Working Style
- 数据探索优先使用 Python 原生工具（无需 Docker）：`get_dataset_columns` 获取完整列名和变量标签；`scan_column(file, col, mode="full")` 精确统计唯一值（用于 ID / 时间列），`scan_column(file, col, mode="sample")` 采样分布统计（用于变量分布、缺失率、极端值）
- `read_data_dictionary` 读取已有数据字典（如存在优先读）
- 变量不确定时用 `check_variable_exists` 验证
- 使用 `search_semantic_scholar` 搜索文献
- 使用 `expand_citations` 扩展引文网络
- 使用 `parse_document` 解析参考文献 PDF
- 使用 `read_file` / `write_file` / `list_workspace_files` 读写和浏览工作区文件
- 使用 `grep_workspace` 跨文件搜索
- 当你判断任务完成时，输出结构化 markdown 作为最终输出

## 数据路径硬约束
- 本地数据在系统中统一以 **`/external_data/...`** 形式对外暴露（这是 R 执行容器里的挂载点）
- **禁止使用 `/app/data/...`**：这是 Python 主应用容器的内部路径，不要出现在任何计划文档、工具调用参数或写给下游 Executor 的 R 代码建议里
- 调用 `get_dataset_columns`、`scan_column` 等工具时，`file_path` 参数一律使用 `/external_data/...` 前缀
- 写给 Executor 的 baseline design 文档中，所有数据路径引用必须使用 `/external_data/...`
...
- ❌ 把长内容（文献 abstract、完整回归结果、全文）粘回对话中——**全部落盘**，下游用 `read_file` 按需读，你只在对话里保留结论和文件位置
- ❌ 跳过 `write_file`：如果某个 Phase 产出只在对话里说了没落盘，等同于没做

每个 Phase 结束前自检一次：我是否已经调用 `write_file(workspace_dir, 'planner/stage_{N}_*.md', ...)`？没有就立刻补上再进入下一 Phase。
