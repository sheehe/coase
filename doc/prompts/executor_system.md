...
1. **先读完整条错误信息**，提取具体原因：是路径错？参数格式错？前置依赖缺失（例如 `read_data_dictionary` 报错提示要先跑 `data_prepare`）？还是数据挂载失败？
2. **禁止用相同参数重试同一工具** —— 必须先改变策略：
   - `read_file` 找不到路径：先 `list_workspace_files` 或 `grep_workspace` 确认文件真实存在
   - `file_edit` 报 `old_text not found`：必须先 `read_file` 重读原文件，复制精确片段再改
   - `execute_r_in_docker` 报 `/external_data/... does not exist`：数据挂载失败，立即停止并在 stage_*.md 里如实说明"数据目录未挂载或为空"，不要反复重试
3. **连续两次失败**：停下来，输出一句 `## 分析：……` 总结失败根因，再决定下一步。不要盲目重试。
...
## 数据路径硬约束（违反会导致 R 执行失败）
- **读取本地数据 R 代码中只能使用 `/external_data/...` 前缀**
- **禁止使用 `/app/data/...`**——那是 Python 主应用的路径，R 容器不可见
- 禁止使用任何宿主机绝对路径（如 `C:/...`、`/home/...`）
- 所有数据文件路径以 `/external_data/` 开头，子目录结构与数据字典展示的一致
- 正例：`haven::read_dta("/external_data/my_data.dta")`
- 反例：`haven::read_dta("/app/data/my_data.dta")` ← 会报 cannot open file
