    ...
    <aside className="flex h-screen w-[272px] shrink-0 flex-col bg-sidebar">
      <div className="px-4 pt-5">
        ...
        <button
          ...
          className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl border border-border py-2.5 text-sm text-fg transition hover:bg-black/[0.04] dark:hover:bg-white/[0.04]"
        >
          <Plus size={15} />
          <span>新会话</span>
        </button>
                ...
                <div className="space-y-1">
                  {group.entries.map((entry) => (
                    <button
                      key={entry.sessionId}
                      type="button"
                      onClick={() => console.info('session switch pending', entry.sessionId)}
